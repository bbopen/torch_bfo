...
class BFO(Optimizer):
    ...
    def __init__(
        ...
        # Set device early from first parameter
        temp_param = next(iter(self.param_groups[0]["params"]))
        self.device = device or temp_param.device
        ...
        self.logger = OptimizationLogger("BFO") if verbose else None
        
        # Compile optimization functions if requested
        if compile_mode and torch.cuda.is_available():
            compile_kwargs = compile_kwargs or {}
            # Default to "default" mode for better compatibility with smaller GPUs
    ...
    def _optimization_step(
        ...
        while improved.any() and (swim_count < max_swim).any():
            ...
            # Update fitness for improved solutions
    ...
    def step(self, closure: Optional[Callable] = None) -> float:
        ...
        if self.verbose:
            logger.info(f"Starting BFO step {self.current_iter}")
        ...
        for elim_iter in range(self.defaults["elim_steps"]):
            for repro_iter in range(self.defaults["repro_steps"]):
                ...
                for i in range(pop_size):
                    ...
                    for p, new_p in zip(self.param_groups[0]["params"], params_list):
                        p.data.copy_(new_p)
                    fitness_values[i] = self._evaluate_closure(closure)
                    
                sorted_idx = fitness_values.argsort()
                half = pop_size // 2
                if half > 0:
                    # Handle odd population sizes correctly
                    # Replace worst half with clones of best half
        ...
        if self.verbose:
            ...
            diversity = self._compute_diversity() if hasattr(self, '_compute_diversity') else None
