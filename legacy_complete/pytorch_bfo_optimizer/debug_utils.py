...
def log_debug(message: str, **kwargs):
    """Helper function for debug logging with optional context"""
...
class DebugContext:
    """Context manager for debug logging with timing and error handling"""
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            self.logger.debug(f"Exiting {self.name} (took {elapsed:.4f}s)")
        else:
            self.logger.error(f"Error in {self.name}: {exc_type.__name__}: {exc_val}")
