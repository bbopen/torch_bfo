...
class BFO(Optimizer):
    ...
    Arguments:
        ...
        domain_bounds (tuple, optional): (min, max) bounds to clamp population to (default: None)
        ...
        try:
            with torch.no_grad():
                ...
                param_list = self._unflatten_group_params(
                    individual, self.state[id(group)]["param_shapes"]
                )
    ...
    def _evaluate_batch_closure(
        ...
        current_fe = (
            self.state[group_id]["function_evaluations"]
        ...
        if (
            ...
            def single_eval(params_flat):
                ...
                param_dict = {
                    name: p for name, p in zip(model.state_dict().keys(), params)
                }
    ...
    def _resize_population(
        ...
        if new_size > current_size:
            ...
            for i in range(current_size):
                fitness[i] = self._evaluate_closure(
                    closure, group, current_population[i]
                )
            ...
            for group in self.param_groups:
                ...
                if group_id in self.state:
                    ...
                    if "momentum_buffer" not in self.state[group_id]:
                        param_vector, _ = self._flatten_group_params(group)
                        self.state[group_id]["momentum_buffer"] = torch.zeros_like(
                            param_vector
                        )
