...
class ChaoticBFO(BFO):
    ...
    def _levy_flight(
        self, size: torch.Size, alpha: float, device: torch.device, dtype: torch.dtype
    ) -> torch.Tensor:
        ...
        try:
            ...
            beta = 1.5
            sigma_u = (
                torch.lgamma(torch.tensor(1 + beta))
                * torch.sin(torch.tensor(np.pi * beta / 2))
                / (
                    torch.lgamma(torch.tensor((1 + beta) / 2))
                    * beta
                    * 2 ** ((beta - 1) / 2)
                )
            ) ** (1 / beta)
            ...
            return (
                step * (torch.rand(size, device=device, dtype=dtype) < 0.5).float() * 2
                - 1
            )

        except Exception as e:
            logger.warning(
                f"Lévy flight calculation failed: {e}, using chaotic fallback"
            )
    ...
    def _optimization_step(
        ...
        for elim_step in range(group["elimination_steps"]):
            for repro_step in range(group["reproduction_steps"]):
                for chem_step in range(group["chemotaxis_steps"]):
                    ...
                    if hasattr(self, "_max_fe") and self._max_fe is not None:
                        remaining_fe = self._max_fe - group_state.get(
                            "function_evaluations", 0
                        )
                        if remaining_fe < pop_size:
                            logger.info(
                                f"Stopping: remaining FE {remaining_fe} < pop_size {pop_size}"
                            )
                            return group_state
                    ...
                    if (
                        hasattr(self, "_max_fe")
                        and self._max_fe is not None
                        and group_state.get("function_evaluations", 0) >= self._max_fe
                    ):
                        logger.info(
                    ...
                    if min_fitness <= self.success_tolerance:
                        logger.info(
                            f"Reached success tolerance {self.success_tolerance}; exiting early."
                        )
                        group_state["best_fitness"] = min_fitness
                        group_state["best_params"] = population[min_idx].clone()
                        return group_state
                    ...
                    if group_state["stagnation_count"] >= 3:
                        ...
                        if num_restart > 0 and remaining_fe > 0:
                            restart_idx = torch.randperm(pop_size, device=self.device)[
                                :num_restart
                            ]
                            domain_min, domain_max = self.domain_bounds
                            new_positions = torch.rand(
                                (num_restart, param_dim),
                                device=self.device,
                                dtype=dtype,
                            )
                            new_positions = (
                                new_positions * (domain_max - domain_min) + domain_min
                            )
                            population[restart_idx] = new_positions
                            group_state["function_evaluations"] += num_restart
                            logger.info(
                                f"Micro-restart: replaced {num_restart} stagnant bacteria"
                            )
                        group_state["stagnation_count"] = 0
                    ...
                    if group["enable_swarming"]:
                        ...
                        movement = (
                            group_state["current_step_size"] * levy_steps
                            + 0.1 * swarming_forces
                        )
                    else:
                    ...
                    while improved.any() and swim_count < group["swim_length"]:
                        ...
                        population[swim_indices] += (
                            group_state["current_step_size"] * levy_steps[swim_indices]
                        )
                ...
                if pop_size > 1:
                    ...
                    if self.enable_crossover:
                        for dst, p_idx in zip(offspring_idx, parents_idx):
                            ...
                            child = torch.where(
                                mask, population[p_idx], population[parent2_idx]
                            )
                            mutation = (
                                torch.randn_like(child)
                                * group_state["current_step_size"]
                                * 0.05
                            )
            ...
            iter_frac = (
                group_state["iteration"] / max_iterations if max_iterations > 0 else 0.0
            )
            ...
            if diversity < diversity_threshold:
                ...
                logger.info(
                    f"Low diversity ({diversity:.4f} < {diversity_threshold:.4f}), triggering forced dispersal"
                )
                ...
                if num_replace > 0:
                    replace_idx = torch.randperm(pop_size, device=self.device)[
                        :num_replace
                    ]
                ...
                domain_min, domain_max = self.domain_bounds
                new_positions = torch.rand(
                    (num_replace, param_dim), device=self.device, dtype=dtype
                )
                ...
                if num_replace > 0:
                    ...
                    logger.info(
                        f"Replaced {num_replace} bacteria with random positions"
                    )
            else:
